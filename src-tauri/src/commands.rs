    ...
    pub fn new() -> Result<Self, String> {
        ...
        let device_id = match db.get_setting("device_id") {
            Ok(Some(id)) if !id.is_empty() => {
                println!(
                    "[Dingo] Loaded persisted device_id: {}",
                    &id[..8.min(id.len())]
                );
                id
            }
...
pub fn init_app(state: State<AppState>) -> Result<InitResult, String> {
    ...
    if existing_user.is_none() {
        let hostname = hostname::get()
            ...
            .unwrap_or_else(|_| "Dingo User".to_string());
        let user = User {
                ...
                Ok(msg) => match &msg {
                    SignalingMessage::ChatMessage {
                                        ...
                                        if avatar.starts_with("filemeta:") {
                                            ...
                                            if parts.len() >= 3 {
                                                ...
                                                match db.set_user_avatar(&from, &url) {
                                                    Ok(_) => println!(
                                                        "[Dingo] Resolved avatar for {}",
                                                        from
                                                    ),
                                                    Err(e) => println!(
                                                        "[Dingo] Failed to resolve avatar: {}",
                                                        e
                                                    ),
                                                }
                        ...
                        match db.create_message(&message) {
                            Ok(_) => println!(
                                "[Dingo] Stored incoming message {}",
                                &id[..8.min(id.len())]
                            ),
                            Err(e) => println!("[Dingo] Failed to store message: {}", e),
                        }
                    ...
                    SignalingMessage::ProfileUpdate {
                        ...
                        let resolved_avatar: Option<String> = if let Some(url) = avatar_url {
                            match db.set_user_avatar(from, &url) {
                                Ok(_) => println!("[Dingo] Updated avatar for {}", from),
                                Err(e) => println!("[Dingo] Failed to set avatar: {}", e),
                            }
                            Some(url.clone())
                            ...
                            if let Some(pc) = signaling.get_peer(&from) {
                                ...
                                match db.set_user_avatar(from, &url) {
                                    Ok(_) => println!("[Dingo] Set avatar (file) for {}", from),
                                    Err(e) => println!("[Dingo] Failed to set avatar: {}", e),
                                }
                                Some(url)
                            } else {
                                ...
                                match db.set_user_avatar(from, &placeholder) {
                                    Ok(_) => {
                                        println!("[Dingo] Stored avatar placeholder for {}", from)
                                    }
                                    Err(e) => println!(
                                        "[Dingo] Failed to store avatar placeholder: {}",
                                        e
                                    ),
                                }
                                Some(placeholder)
                    ...
                    SignalingMessage::GroupCreated {
                        ...
                        println!("[Dingo] Received group created from {} ({})", from, id);
                        ...
                        match db.create_group(&group) {
                            Ok(_) => println!("[Dingo] Stored group {}", &id[..8.min(id.len())]),
                            Err(e) => println!("[Dingo] Failed to store group: {}", e),
                        }
                        for (i, uid) in member_ids.iter().enumerate() {
                            ...
                            match db.add_group_member(&gm) {
                                Ok(_) => {}
                                Err(e) => println!("[Dingo] Failed to add group member: {}", e),
                            }
                        }
                    ...
                    SignalingMessage::GroupChatMessage {
                        ...
                        println!(
                            "[Dingo] Received group chat message from {} in group {}",
                            sender_name,
                        ...
                        match db.send_group_message(&gmsg) {
                            Ok(_) => {
                                println!("[Dingo] Stored group message {}", &id[..8.min(id.len())])
                            }
                            Err(e) => println!("[Dingo] Failed to store group message: {}", e),
                        }
                    ...
                    SignalingMessage::MeetingChatMessage {
                        ...
                        println!(
                            "[Dingo] Received meeting chat from {} (session {})",
                            sender_name,
                    ...
                    SignalingMessage::GroupMemberAdded {
                        ...
                        println!(
                            "[Dingo] Group member added: {} to group {}",
                            username,
                            &group_id[..8.min(group_id.len())]
                        );
                        let gm = GroupMember {
                    ...
                    SignalingMessage::GroupMemberRemoved {
                        ...
                        println!(
                            "[Dingo] Group member removed: {} from group {}",
                            user_id,
                            &group_id[..8.min(group_id.len())]
                        );
...
/// Download an avatar from remote URL and cache locally in Documents/Dingo/avatars/
...
pub fn download_and_cache_avatar(
    ...
    match state.db.set_user_avatar(&device_id, &file_url) {
        Ok(_) => println!(
            "[Dingo] Cached avatar for {} at {} (served as {})",
            device_id,
            file_path.display(),
            file_url
        ),
        Err(e) => println!("[Dingo] Warning: failed to update avatar in DB: {}", e),
    }
...
pub fn register_local_avatar(
    ...
    match state.db.set_user_avatar(&device_id, &local_url) {
        Ok(_) => println!(
            "[Dingo] Registered local avatar for {} as {}",
            device_id, local_url
        ),
        Err(e) => println!("[Dingo] Warning: failed to update avatar in DB: {}", e),
    }
...
pub fn auto_download_file<R: Runtime>(
    ...
    // Also save to organized downloads: Dingo/Downloads/<sender_name>/<type>/<file_name>
...
/// Get the base Dingo downloads directory
#[tauri::command]
pub fn get_dingo_downloads_base(state: State<AppState>) -> String {
    state
        .file_transfer
        .get_downloads_dir()
        .to_string_lossy()
