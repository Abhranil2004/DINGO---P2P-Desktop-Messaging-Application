// src/hooks/useApp.js
// Core application hooks for Dingo
...
export function useApp() {
    ...
    const autoDownloadFileMessage = useCallback(async (msg) => {
        ...
        try {
            ...
            if (!senderIp) {
                console.warn('[Dingo] Auto-download: no sender IP for', msg.sender_id?.slice(0, 8));
                return;
            }

            const url = `http://${senderIp.split(':')[0]}:${info.port}/file/${info.fileId}`;
            console.log('[Dingo] Auto-downloading file:', info.fileName, 'from', url);
                ...
                .then(async () => {
                    ...
                    try {
                        ...
                        if (dataUrl && typeof window !== 'undefined') {
                            window.dispatchEvent(new CustomEvent('dingo:file-downloaded', {
                                detail: { fileId: info.fileId, dataUrl }
                            }));
                        }
                    } catch { /* ignore — UI will lazy-load via effect */ }
                })
                .catch(e => console.warn('[Dingo] Auto-download failed:', e));
    ...
    useEffect(() => {
        ...
        const wb = setTimeout(() => {
            if (!finished) {
                console.warn('[Dingo] bootstrap still not complete after 10s');
                api.appendDevLog && api.appendDevLog('[Dingo] bootstrap still not complete after 10s').catch(() => { });
            }
        }, 10000);

        (async () => {
            try {
                console.log('[Dingo] bootstrap started');
                api.appendDevLog && api.appendDevLog('[Dingo] bootstrap started').catch(() => { });
                ...
                try {
                    ...
                    api.appendDevLog && api.appendDevLog('[Dingo] file server port=' + (port || 0)).catch(() => { });
                } catch { /* ignore */ }
    ...
    const flushPendingMessages = useCallback(async (peerDeviceId) => {
        ...
        if (now - lastFlush < 30000) {
            console.debug('[Dingo] Skipping flush for', peerDeviceId.slice(0, 8), '- rate limited');
            return;
        }

        // Prevent concurrent flushes for same peer
        if (flushInProgressRef.current.has(peerDeviceId)) {
            console.debug('[Dingo] Skipping flush for', peerDeviceId.slice(0, 8), '- already in progress');
            return;
        }
        ...
        try {
            ...
            for (const p of toSend) {
                try {
                    ...
                    console.warn('[Dingo] Failed to deliver pending message', p.id, err);
                }
            }
            ...
            console.warn('[Dingo] flushPendingMessages error:', err);
        } finally {
            flushInProgressRef.current.delete(peerDeviceId);
        }
    }, [refreshLastMessages]);
    ...
    useEffect(() => {
        ...
        unsubs.push(api.onPeerDiscovered(async peer => {
            ...
            console.debug('[Dingo][debug] onPeerDiscovered — id=%s incomingUsername=%s existingUsername=%s', peer.device_id, peer.username, existingUser?.username);
        ...
        unsubs.push(api.onPeerUpdated(peer => {
            ...
            const existingUser = peerMapRef.current.get(peer.device_id);
            console.debug('[Dingo][debug] onPeerUpdated — id=%s incomingUsername=%s existingUsername=%s ip=%s port=%s', peer.device_id, peer.username, existingUser?.username, ip, port);
            ...
            if (peer.username && existingUser?.username && peer.username !== existingUser.username) {
                console.debug('[Dingo][debug] onPeerUpdated: suppressed presence username update — incoming=%s existing=%s id=%s', peer.username, existingUser.username, peer.device_id);
            }
            ...
            if (!resolvedUsername && peer.username) {
                if (!prevPresenceLocked) {
                    ...
                    console.debug('[Dingo][debug] onPeerUpdated: accepted presence-derived username=%s for id=%s', resolvedUsername, peer.device_id);
                } else {
                    // Already locked to a presence-derived username — ignore changes
                    if (prev.username && prev.username !== peer.username) {
                        console.debug('[Dingo][debug] onPeerUpdated: ignored changed presence username (locked) — incoming=%s locked=%s id=%s', peer.username, prev.username, peer.device_id);
                    }
                }
            }
        ...
        unsubs.push(api.onSignalingMessage(msg => {
            ...
            if (msg?.type === 'DeliveryAck' && msg?.from && msg?.message_id) {
                ...
                try {
                    if (typeof window !== 'undefined') {
                        window.dispatchEvent(new CustomEvent('dingo:pending-delivered', { detail: { peerId, messageId } }));
                    }
                } catch (e) { /* ignore */ }
            ...
            if (msg?.type === 'ProfileUpdate' && msg?.from) {
                ...
                console.debug('[Dingo][debug] ProfileUpdate received — peerId=%s msg.username=%s existingUsername=%s avatar=%s', peerId, msg.username, existingUser?.username, msg.avatar_url ? 'yes' : 'no');
                ...
                console.debug('[Dingo][debug] ProfileUpdate applied — peerId=%s newUsername=%s avatar_path=%s', peerId, updated.username, updated.avatar_path ? 'set' : 'none');
                setPeers(Array.from(peerMapRef.current.values()));
            ...
            if (user.avatar_path.startsWith('http://') || user.avatar_path.startsWith('https://')) {
                ...
                tasks.push((async () => {
                    try {
                        ...
                        console.warn(`[Dingo] Failed to cache avatar for ${user.username}:`, err);
                    }
                })());
            }
        }
        await Promise.all(tasks).catch(err => console.warn('[Dingo] Avatar caching batch failed:', err));
    ...
    useEffect(() => {
        const handleNotificationClick = (event) => {
            ...
            console.log('[Dingo] Notification clicked - opening chat with', senderName, '(', senderId, ')');

            // Dispatch event for chat page to handle navigation
            if (typeof window !== 'undefined') {
                window.dispatchEvent(new CustomEvent('dingo:open-chat', {
        ...
        const handleWindowFocus = () => {
            ...
            if (lastNotif && lastNotif.senderId) {
                console.log('[Dingo] App focused - checking for pending notifications');
        ...
        if (typeof window !== 'undefined') {
            window.addEventListener('dingo:notification-clicked', handleNotificationClick);
            window.addEventListener('focus', handleWindowFocus);
            return () => {
                window.removeEventListener('dingo:notification-clicked', handleNotificationClick);
    ...
    const saveAvatar = useCallback(async (imageData) => {
        ...
        try {
            ...
            console.warn('[Dingo] Avatar broadcast failed:', e);
        }
        return result;
    ...
    useEffect(() => {
        if (typeof window !== 'undefined' && import.meta.env.DEV) {
            ...
            window.dingoFlushPending = (peerId) => flushPendingMessages(peerId).catch(e => console.warn('[Dingo] dingoFlushPending failed', e));
        }
...
export function useChat() {
    ...
    useEffect(() => {
        ...
        window.addEventListener('dingo:pending-delivered', handler);
        return () => window.removeEventListener('dingo:pending-delivered', handler);
    }, []); // Empty deps — uses ref
