...
// Native Windows Notifications for Dingo with Avatar Support
...
async function showNotification(body, options = {}) {
    ...
    if (!isTauri) {
        console.log(`[📬 Notification from Dingo] ${body}`);
        if ('Notification' in window && Notification.permission === 'granted') {
            const notification = new Notification('Dingo Messenger', { body, ...options });
            if (options.onClick) {
    ...
    try {
        ...
        // Send native Windows notification with "Dingo Messenger" title
        // Format: "Dingo Messenger" with body like "John Doe: Hey bro are you free?"
        await sendNotification({
            title: 'Dingo Messenger',
            body: body,
            tag: notificationId
        });
 ...
 * Displays as: "Dingo Messenger" | "John Doe: Hey bro are you free?"
