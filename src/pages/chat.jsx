...
export default function ChatPage() {
    ...
    const loadFileDataUrl = useCallback(async (fileId) => {
        ...
        try {
            ...
            console.warn('[Dingo] Failed to load file directly:', err);
            return null;
        }
    }, [loadedFileUrls]);
    ...
    const handleImageError = useCallback(async (msg) => {
        try {
            ...
            if (!fileId) {
                console.warn('[Dingo] handleImageError: no fileId in message');
                return;
            }
            ...
            try {
                ...
                if (dataUrl) {
                    loadedFileIdsRef.current.add(fileId);
                    setLoadedFileUrls(prev => ({ ...prev, [fileId]: dataUrl }));
                    console.log('[Dingo] Loaded file via direct read for', fileId);
                    loadingFilesRef.current.delete(fileId);
                    return;
                }
            } catch (readErr) {
                console.warn('[Dingo] Direct read failed for', fileId, readErr);
            }

            // If no sender IP or port, we can't fetch remotely
            if (!senderIp || !port) {
                console.warn('[Dingo] handleImageError: no sender IP/port available for', msg.sender_id);
                loadingFilesRef.current.delete(fileId);
                return;
            }
            ...
            console.log('[Dingo] Auto-downloading file:', info.fileName, 'from', remoteUrl);
            try {
                ...
                if (dataUrl) {
                    loadedFileIdsRef.current.add(fileId);
                    setLoadedFileUrls(prev => ({ ...prev, [fileId]: dataUrl }));
                    console.log('[Dingo] File downloaded and loaded directly');
                }
            } catch (dlErr) {
                console.warn('[Dingo] Auto-download failed:', dlErr);
            }

            loadingFilesRef.current.delete(fileId);
        } catch (e) {
            console.error('[Dingo] Image error handler failed:', e);
        }
    ...
    const handleDownload = useCallback(async (url, fileName) => {
        try {
            ...
            console.error('[Dingo] Save failed:', e);
    ...
    const handleOpenFileLocation = useCallback(async (msg) => {
        try {
            ...
            console.warn('[Dingo] Open file location failed:', e);
                    ...
                    try {
                        if (msg.message_type === 'image' || msg.message_type === 'video' || msg.message_type === 'file') {
                            ...
                            if (info && info.fileId) {
                                ...
                                // becomes available and `dingo:file-downloaded` will update loadedFileUrls
    ...
    useEffect(() => {
        ...
        if (typeof window !== 'undefined') {
            window.addEventListener('dingo:open-chat', handleOpenChat);
            return () => {
                window.removeEventListener('dingo:open-chat', handleOpenChat);
            };
        }
    ...
    useEffect(() => {
        ...
        window.addEventListener('dingo:file-downloaded', handler);
        return () => window.removeEventListener('dingo:file-downloaded', handler);
    }, []);
